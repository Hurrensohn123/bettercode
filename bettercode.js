...
.custom-form_grid {
  ...
  row-gap: 2.5rem; /* mehr vertikaler Abstand */
}
...
.custom-label {
  ...
  font-size: 1.2rem;
  line-height: 1.1;
  margin-bottom: 0.6rem; /* Abstand Label → Input */
  color: #000;
}
...
.custom-form_btn {
  ...
  border-radius: 8px;
  cursor: pointer;
  min-width: 90px;      /* verhindert Schrumpfen */
  text-align: center;   /* Text sauber zentriert */
  white-space: nowrap;  /* kein Umbruch bei senden… */
}

.custom-form_btn:hover {
  opacity: 0.9;
}
...
/* Erfolg */
.custom-form_status.is-success {
  color: #1c1c1c;
}

/* Fehler */
.custom-form_status.is-error {
  color: #c40000;
}
}
`;
...
function initContactForm() {
  ...
  const statusEl = document.querySelector('.custom-form_status');
  ...
  // doppelte Listener verhindern (Barba-safe)
  if (button.__formBound) return;
  button.__formBound = true;
  ...
  button.addEventListener("click", async (e) => {
    ...
    if (!name || !email || !message) {
      showStatus("Bitte alle Felder ausfüllen.", "is-error");
      return;
    }
    ...
    try {
      const res = await fetch("https://formspree.io/f/maqjnalq", {
        method: "POST",
        headers: {
